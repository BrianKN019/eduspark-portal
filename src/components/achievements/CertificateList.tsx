...
const CertificateList: React.FC<CertificateListProps> = ({ certificates }) => {
  const handleDownload = (certificate: Certificate) => {
    try {
      ...
      toast.success(`${certificate.name} downloaded successfully!`);
    } catch (error) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {certificates.map((certificate) => (
        <Card key={certificate.id} className="neumorphic-card neumorphic-convex overflow-hidden">
          <div className="bg-gradient-to-r from-yellow-100 to-yellow-200 dark:from-yellow-800 dark:to-yellow-900 h-2"></div>
          ...
          <CardContent>
            ...
            <Button 
              ...
              className="w-full neumorphic-button neumorphic-convex bg-gradient-to-r from-yellow-400 to-yellow-600 hover:from-yellow-500 hover:to-yellow-700"
            >
